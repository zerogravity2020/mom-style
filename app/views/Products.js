...
class Products extends Component {
	...
	componentDidMount() {
		...
		ProductModule.loadProductsCategory(categoryId)
    }
	...
	hideMenu = (type, params) => {
		...
		ProductModule.loadProductsCategory(`${categoryId}&${params}`)
	}
		...
		return (
			<View style={theme === 'dark' ? styles.pageDark : styles.page}>
					...
					<Ripple style={[styles.actionsBtn, styles.actionBtnWide]}
						...
						<View style={styles.actionsTextWrap}>
							<Menu ref="menu"
								...
								<MenuItem textStyle={theme === 'dark' ? styles.textColorDark : styles.textColor} onPress={() => this.hideMenu('новинки', 'orderby=date')}>новинки</MenuItem>
				...
				<Modal visible={filterModal}
					...
					<ModalContent style={styles.modalFilterContent}>
			            <Text style={[styles.modalFilterTitle, theme === 'dark' ? styles.textColorDark : styles.textColor]}>Фільтр</Text>
			            <ScrollView style={styles.modalFilterBody} showsVerticalScrollIndicator={false}>
					    			...
					    			<View style={[styles.cardAttributesList, {marginTop: 10}]}>
						    			{attribute.options.map(option => 
						    				<Ripple key={`attribute-${attribute.id}-option-${option.slug}`}
						    						...
						    						rippleCentered={true}
						    						onPress={() => ProductModule.changeAttribute(attribute.id, option.id)}>
			            ...
			            <View style={[styles.modalFilterRow, {marginBottom: 0}]}>
		            		<Button caption="Очистити"
				            		...
				            		onPress={() => {ProductModule.resetFilter(); ProductModule.loadProductsCategory(`${categoryId}&${filterQuery}`)}} />
				            <Button caption="Застосувати"
		            				customStyle={styles.modalFilterCol}
			            			onPress={() => {this.openFilter(false); ProductModule.loadProductsCategory(`${categoryId}&${filterQuery}`)}} />
			            </View>
